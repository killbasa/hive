...
export const reference: FastifyPluginCallback = (server, _, done) => {
	server.get('/spec.json', async (_, reply): Promise<void> => {
	...
	server.get('/reference', async (_, reply): Promise<void> => {
		await reply
			.headers({
